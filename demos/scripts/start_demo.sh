...
DEMO_RUN_DIR="${REPO_ROOT}/demos/run"
RELAY_ENV_FILE="${REPO_ROOT}/demos/relay/.env"
RELAY_ENV_TEMPLATE="${REPO_ROOT}/demos/relay/.env.example"
...
acp_cmd() {
  ...
  if command -v acp >/dev/null 2>&1; then
    ...
    PYTHONWARNINGS="ignore::RuntimeWarning" PYTHONPATH="${ACP_PYTHONPATH}" "${PYTHON_BIN}" -m acp_cli.main "${insecure_flags[@]}" "$@"
  fi
}
