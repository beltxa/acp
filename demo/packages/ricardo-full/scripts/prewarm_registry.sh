...
JOHN_STORAGE="${JOHN_STORAGE:-${REPO_ROOT}/demo/identities/john}"
RICARDO_STORAGE="${RICARDO_STORAGE:-${REPO_ROOT}/demo/identities/ricardo}"
...
ensure_identity() {
  ...
  if ! acp_cmd --storage-dir "${storage_dir}" identity show --agent-id "${agent_id}" >/dev/null 2>&1; then
    ...
    echo "Create demo identities first (see demo/scripts/start_demo.sh init-identities)." >&2
    return 1
  fi
}
